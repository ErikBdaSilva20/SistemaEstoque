...
// No aggregate RPC/view in generic mode -- ABC/turnover/stockout are computed
// client-side from the raw product + movement lists. Pure so it's testable
// without mocking React Query; `useReports` is a thin wrapper around this.
export function computeReports(
  ...
  const since = new Date(now - periodDays * 24 * 60 * 60 * 1000).toISOString();
    ...
    for (const m of ms) {
      ...
      if (balance <= 0) {
        stockouts.push({
          ...
          daysAgo: Math.floor((now - new Date(m.created_at).getTime()) / 86_400_000),
        });
        break;
      }
    }
  ...
  for (const p of products) {
    if (Number(p.current_stock) <= 0 && !stockouts.some((s) => s.productId === p.id)) {
      stockouts.push({
        ...
        name: p.name,
        unit: p.unit,
        stockoutAt: new Date(now).toISOString(),
        daysAgo: 0,
      });
    }
  }
  ...
  return {
    ...
    totals: {
      ...
      lowStockCount: products.filter((p) => Number(p.current_stock) < Number(p.min_stock)).length,
